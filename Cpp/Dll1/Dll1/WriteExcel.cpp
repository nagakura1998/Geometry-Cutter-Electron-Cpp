	...
	while (getline(MyReadFile, myText)) {
		...
		if (data[0] == "facet" && data[1] == "normal") {
			if (data.size() != 5 || bInsideLoop || iInsideLoopCounter) {
				std::cout << "This mesh is corrupted at line " << line << std::endl;
				return;
			}
			for (int i = 2; i < 5; i++) {
		...
		if (data[0] == "vertex") {
			if (!bInsideLoop && data.size() != 4) {
				std::cout << "This mesh is corrupted" << std::endl;
				return;
			}
			Vector3f buff;
			for (int i = 0; i < 3; i++) {
...
int WriteXLSX(char* cadFile, char* logDir, bool bX, bool bY, bool bZ, float fX, float fY, float fZ)
{
    cout << "Data is written to " << logDir << endl;
	CutPlaneOpt opt;
	opt.bCutByX = bX;
	opt.bCutByY = bY;
	opt.bCutByZ = bZ;
	...
	auto preprocessData = new DataPreprocessing(cadFile);
	preprocessData->Preprocessing();

	auto secGen = new SectionDataGenerator(preprocessData->GetReader(), opt);
	secGen->Generate();

	if (opt.bCutByX) {
		if (!DataWriter::WriteSectionDataXlsx(logDir, "X", 1, 2, secGen->intersect_x_order)) {
			std::cout << "Output Section X Data to Logging Directory: Save XLSX failed!" << std::endl;
		}
		else {
			std::cout << "Output Section X Data to Logging Directory: Done!" << std::endl;
		}
	}
	if (opt.bCutByY) {
		if (!DataWriter::WriteSectionDataXlsx(logDir, "Y", 0, 2, secGen->intersect_y_order)) {
			std::cout << "Output Section Y Data to Logging Directory: Save XLSX failed!" << std::endl;
		}
		else {
			std::cout << "Output Section Y Data to Logging Directory: Done!" << std::endl;
		}
	}
	if (opt.bCutByZ) {
		if (!DataWriter::WriteSectionDataXlsx(logDir, "Z", 0, 1, secGen->intersect_y_order)) {
			std::cout << "Output Section Z Data to Logging Directory: Save XLSX failed!" << std::endl;
		}
		else {
			std::cout << "Output Section Z Data to Logging Directory: Done!" << std::endl;
		}
	}
